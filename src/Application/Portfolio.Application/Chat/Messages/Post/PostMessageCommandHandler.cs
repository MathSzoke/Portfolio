...
) : ICommandHandler<PostMessageCommand, ChatMessageResponse>
{
    public async Task<Result<ChatMessageResponse>> Handle(PostMessageCommand cmd, CancellationToken ct)
    {
        ...
        var message = ChatMessage.Create(session.Id, cmd.Content, senderEnum, currentUserId);
        db.ChatMessages.Add(message);

        if (isRecipient) session.LastRecipientSeenAt = DateTime.UtcNow;
        else session.LastSenderSeenAt = DateTime.UtcNow;
        ...
        if (!isRecipient)
        {
            ...
            var shouldAgentReply = !recipientOnline || (recipientUser?.Status != Domain.Users.Enums.UserStatus.Available);
            ...
            if (shouldAgentReply)
            {
                ...
                {
                    var bot = ChatMessage.Create(session.Id, replyText, Sender.System, session.RecipientId);
