...
import { useWindowEvent } from '../../services/hooks/useWindowEvent.js';
import getApiClient from '../../services/apiClient';

const useStyles = makeStyles({
    container: {
        position: 'fixed', right: '24px', bottom: '88px', zIndex: 9999, width: '85%', maxWidth: '800px', height: '75%', maxHeight: '600px',
        display: 'flex', background: tokens.colorNeutralBackground1, border: `1px solid ${tokens.colorNeutralStroke2}`,
        borderRadius: '18px', boxShadow: '0 4px 32px #0002'
    },
    sidebar: {
        width: '35%', background: tokens.colorNeutralBackground2, borderTopLeftRadius: '18px', borderBottomLeftRadius: '18px',
        borderRight: `1px solid ${tokens.colorNeutralStroke2}`, display: 'flex', flexDirection: 'column', gap: '12px', padding: '20px 12px 12px'
    },
    userCard: { width: '100%', borderRadius: '14px', gap: '12px', display: 'flex', flexDirection: 'column' },
    listItem: {
        width: '100%', borderRadius: '12px', padding: 0, display: 'flex', alignItems: 'center', cursor: 'pointer',
        border: `1px solid ${tokens.colorNeutralStroke2}`, overflow: 'hidden', ':hover': { backgroundColor: tokens.colorNeutralBackground2Hover, transform: 'scale(1.02)' }
    },
    listItemBtn: { width: '100%', justifyContent: 'flex-start', padding: '6px 8px', height: 'auto' },
    chatArea: {
        flex: 1, display: 'flex', flexDirection: 'column', background: tokens.colorNeutralBackground1Hover,
        borderTopRightRadius: '18px', borderBottomRightRadius: '18px'
    },
    topBar: {
        height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', padding: '0 10px',
...
export function ChatDialog({ apiBase, open, onClose, onSend, onStart, started, isOwner = false }) {
    ...
    useWindowEvent('chat-bubbles-updated', () => {
        if (endRef.current) endRef.current.scrollIntoView({ behavior: 'smooth' });
    });

    useWindowEvent('chat-bubbles-thinking', (e) => {
        const sid = e?.detail?.sessionId;
        const resolved = activeSessionId || sessionId;
        if (sid && resolved && sid === resolved) setThinking(!!e.detail.thinking);
    });
    ...
    useEffect(() => {
        ...
        if (!stickyRef.current && userInfo) {
            const id = dispatchToast(
                <Toast>
                    <ToastTitle action={<ToastTrigger><Link><Dismiss24Regular /></Link></ToastTrigger>}>{t('common.aiWarn')}</ToastTitle>
                </Toast>,
                ...
                {!isOwner && !showStart && (
                    <div className={s.inputBar}>
                        <ChatInput
                            value={input}
                            onChange={setInput}
                            onSend={async (msg) => { onSend(msg); setInput(''); setTimeout(() => endRef.current?.scrollIntoView({ behavior: 'smooth' }), 0); }}
                            disabled={thinking}
                        />
                    </div>
                )}
