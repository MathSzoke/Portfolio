...
using Portfolio.Application.Auth.LoginEmail;

namespace Portfolio.Api.Endpoints.Auth;

internal sealed class LoginEmail : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("api/v1/auth/login", async (
                [FromBody] LoginEmailRequest request,
                [FromServices] ICommandHandler<LoginEmailCommand, AuthResponse> handler,
                IConfiguration cfg,
                HttpContext http,
                CancellationToken ct) =>
            {
                var cmd = new LoginEmailCommand(request.Email, request.Password);
